...
class TagtlyManager {
  ...
  /// for all until [emailPattern]
  static Map<RegExp, Color> get _regsHighlight => {
        urlPattern: TextColor.url,
        hashtagPattern: TextColor.hashtag,
        twitterPattern: TextColor.twitter,
        facebookPattern: TextColor.facebook,
        instagramPattern: TextColor.insta,
        datePattern: TextColor.date,
        timePattern: TextColor.time,
      };
  ...
  static TextType _textType(String text) {
    bool _isUrl = urlPattern.hasMatch(text);
    bool _isHashtag = hashtagPattern.hasMatch(text);
    bool _isEmail = emailPattern.hasMatch(text);
    bool _isTwitter = twitterPattern.hasMatch(text);
    bool _isFacebook = facebookPattern.hasMatch(text);
    bool _isInstagram = instagramPattern.hasMatch(text);
    bool _isDate = datePattern.hasMatch(text);
    bool _isTime = timePattern.hasMatch(text);
    if (_isEmail) {
  ...
  /// such as [urlPattern], [hashtagPattern], [emailPattern] and more pattern..
  /// explore it below 😄
  ///
  ...
  static RegExp get hashtagPattern => RegExp("#+($patternStr+)");
  static RegExp get urlPattern =>
      RegExp(r'(?:(?:https?|ftp):\/\/)+[\w/\-?=%.]+\.[\w/\-@?;&=%.]+');
  static RegExp get timePattern =>
      RegExp(r'([0-1][0-9]|[٠-١][٠-٩]|2[0-3]||٢[٠-٣]):([0-5][0-9]|[٠-٥][٠-٩])');
  static RegExp get datePattern => RegExp(
      '(0[1-9]|٠[١-٩]|1[0-9]|١[١-٩]|2[0-9]|٢[٠-٩]|3[0-1]|٣[٠-١])+[-|.|/]+(0[1-9]|٠[١-٩]|1[0-2]|١[٠-٢])+[-|.|/]+(200[0-9]|٢٠٠[٠-٩]|201[0-9]|٢٠١[٠-٩]|202[0-9]|٢٠٢[٠-٩])');
  static RegExp get emailPattern => RegExp(r'\b[\w\.-]+@[\w\.-]+\.\w{2,4}\b',
      caseSensitive: false, multiLine: true);
  static RegExp get twitterPattern =>
      RegExp(r'(?:(?:twitter?|tw): @)+[\w/\-_?=%.]+');
  static RegExp get facebookPattern =>
      RegExp(r'(?:(?:facebook?|fb?|Facebook): @)+[\w/\-_?=%.]+');
  static RegExp get instagramPattern =>
